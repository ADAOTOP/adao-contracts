...
async function main() {
    ...
    const c = await ethers.getContractAt("DappsStaking","0x0000000000000000000000000000000000005001");
    // const r = await c.bond_and_stake(astarCommuContracct, "500000000000000000000");
    const r = await c.read_staked_amount(officialAccount);
    // const r = await c.claim_staker(astarCommuContracct);
    // const r = await c.unbond_and_unstake(astarCommuContracct, "1000000000000000000");
